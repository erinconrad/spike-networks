function metric_fig(windows, not_a_spike)
        ...
        for i = 1:length(pt_listing)
            ...
            for f = 1:nfreq
                ...
                if contains(fname,'not') == 0
                    stats(network_count).time(time_count).freq(f).F_all(pt_idx,:,1) = mean(sim.freq(f).ge.data,1);
                else
                    stats(network_count).time(time_count).freq(f).F_all(pt_idx,:,2) = mean(sim.freq(f).ge.data,1);
                end
            end
...
if not_a_spike
    print(gcf,[out_folder,'metric_change_not_spike'],'-depsc');
else
