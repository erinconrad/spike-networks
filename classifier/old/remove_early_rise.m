            ...
            for f = 1:length(metrics(n).time(t).freq)
                ...
                for fn = 1:length(fnames)
                    ...
                    if rm_rise == 1
                        ...
                        all_t_all_times = zeros(length(curr_met.pt),length(curr_met.pt(1).times));
                    ...
                    for p = 1:length(curr_met.pt)
                        ...
                        for sn = 1:length(snames)
                            ...
                            for s = 1:size(sp_or_not.data,1)
                                ...
                                elseif comp_points == 2
                                    zdat = ((data-first_non_nan_data)./abs(first_non_nan_data))';
                        ...
                        % Compare zs between spikes and not at each time
                        % point with two sample t-test
                        sp =  metrics(n).time(t).freq(f).(met).pt(p).spike.zs;
                        not = metrics(n).time(t).freq(f).(met).pt(p).not.zs;
                        metrics(n).time(t).freq(f).(met).pt(p).all_times_t = zeros(1,size(sp,2));
                        for tt = 1:size(sp,2)
                            [~,~,~,st] = ttest2(sp(:,tt),not(:,tt));
                            metrics(n).time(t).freq(f).(met).pt(p).all_times_t(tt) = st.tstat;
                        end
                       
                        all_t_all_times(p,:) = metrics(n).time(t).freq(f).(met).pt(p).all_times_t;
                        metrics(n).time(t).freq(f).(met).all_t_all_times = all_t_all_times;
                    end
