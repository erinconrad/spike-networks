function spike_networks_test(whichPts)

%{
The purpose of this script is to test a paradoxical finding for HUP070
where when I use 0.5 second time windows with a flexible definition of the
spike window (tighter_networks) there appears to be a rise in various network parameters
beginning about 2 seconds pre-spike, but when I use a 0.25 second window
with an inflexible definition of the spike window (spike_networks) there
appears to be no rise. 

So here, I am using an inflexible definition and a 0.5 second time window.

If there is a rise, then this means that somehow I detect a rise with a
broader spike window (which could suggest that there is indeed a rise but
there is not enough signal in 0.25 seconds to detect it). If there is no
rise, then it suggests that somehow the flexbile window makes it appear as
though there is a rise. I don't know how it would do that.
%}
...
for whichPt = whichPts
    ...
    for f = 1:10
        ...
        meta_file = [out_folder,sprintf('adj_test_%d.mat',f)];
